...
namespace MasterUniversityRelational.API.Services
{
    public partial class FacultyService : IFacultyService
    {
        ...
        public async Task<FacultyData> GetByIdAsync(Guid id)
        {
            try
            {
                var data = await _dataService.Get<FacultyData>("sp_GetFacultyByID", new { id = id.ToString() }, CommandType.StoredProcedure);
                return data;
            }
            catch (Exception ex)
            {
        ...
        public async Task<FacultyData> Save(FacultyData facultyData)
        {
            ...
            try
            {
                var data = await _dataService.SaveOne("sp_SaveFaculty", facultyData, false, CommandType.StoredProcedure);
                return facultyData;
            }
            catch (Exception ex)
            {
        ...
        public async Task<FacultyData> Update(FacultyData facultyData)
        {
            try
            {
                var data = await _dataService.RunQuery("sp_UpdateFaculty", facultyData, false, CommandType.StoredProcedure);
                return facultyData;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> Delete(Guid id)
        {
            try
            {
                var data = await _dataService.RunQuery("sp_DeleteFaculty", new { id = id.ToString() }, false, CommandType.StoredProcedure);
                return true;
            }
            catch (Exception ex)
            {
